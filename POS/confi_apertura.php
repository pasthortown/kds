...
if (htmlspecialchars(isset($_GET["grabaperiodo"]))) {
    ...
    $lc_condiciones[2] = preg_replace('[\s+]', '', htmlspecialchars($_GET['usr_usuario'])); //$_GET['usr_usuario'];  //preg_replace(�[\s+]�,�, $cadena);
...
if (htmlspecialchars(isset($_GET["validaAccesoPerfil"]))) {
    ...
    if ($tieneTransferencia == 1) {
        ...
        try {
            ...
            $datosWebservice=$servicioWebObj->retorna_WS_Trans_Venta_ActualizacionPrecios($restaurante);
            $url=$datosWebservice["urlwebservice"]. "?bdd=" . $lc_condiciones[6];
            ...
            if (curl_errno($ch) || $http_status === 400 || $http_status === 404 || $http_status === 0) {
               ...
               // $origenDestino = " Destino";
                ...
                if (empty($respuesta)) {
                    $estado = false;
                    $origenDestino = " Ningún registro actualizado (El Ws en el destino no devolvió  valores)";
                }
            }
        ...
        if ($estado) { //$estado
            try {
                $restaurante = $_SESSION['rstId'];
                $datosWebservice=$servicioWebObj->retorna_WS_Trans_Venta_RetornaPrecios($restaurante);
                $url_getPrecio=$datosWebservice["urlwebservice"]. "?cdn_id=" . $lc_condiciones[4] . "&rst_id=" . $lc_condiciones[5] . "&bdd=" . $lc_condiciones[6];
                ...
                if (curl_errno($ch1)  || $http_status1 === 400|| $http_status1 === 404|| $http_status1 === 0) {
                   ...
                   // $origenDestino = " Origen";
                    ...
                    if (empty($respuesta1)) {
                         $origenDestino = " Ningún registro actualizado (El Ws en el origen no devolvió  valores)";
                        $estado = false;
                    } else {
