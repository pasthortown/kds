...
class Tictuk
{
    ...
    function solicitarToken()
    {
        ...
        $urlServicio = $this->serviceUrl->retorna_rutaWS($this->idRestaurante, "TICTUK", "AUTHENTICATION");
    ...
    function cambioEstado($idFactura, $estado, $idUsuario, $idEstacion, $idMotivoAnulacion, $observacion)
    {
        ...
        $urlServicio = $this->serviceUrl->retorna_rutaWS($this->idRestaurante, "TICTUK", "CAMBIO ESTADO");
        $this->request->url = $urlServicio["urlwebservice"];
    ...
    function anularPedidoDuna( $idFactura, $estado, $idUsuario, $idEstacion, $idMotivoAnulacion, $observacion ) {
        ...
        if ($urlValido && $autentication["registros"] > 0) {
            ...
            if (isset($data) && isset($data->code) && $data->code === "success") {
                $this->config->guardarAuditoriaEstadosApp('1',$urlServicio["urlwebservice"], 'TICTUK - Cambio a Estado: ' . $estado . ', CodigoApp: ' . $codigoApp.' CodigoExterno: '.$codigoExterno, $this->response->httpStatus, json_encode($data).'Codigo - status: '.$codigoApp.'-'.$estado);
            } else {
                $this->config->guardarAuditoriaEstadosApp('1',$urlServicio["urlwebservice"], 'TICTUK - Cambio a Estado: ' . $estado . ', CodigoApp: ' . $codigoApp.' CodigoExterno: '.$codigoExterno, 'ERROR', json_encode($data). ' - '. json_encode($this->response).'Codigo - status: '.$codigoApp.'-'.$estado );
            }

            return json_encode($data);
        }else {
            $this->config->guardarAuditoriaEstadosApp('1',$urlServicio["urlwebservice"], 'TICTUK - Cambio a Estado: ' . $estado . ', CodigoApp: ' . $codigoApp.' CodigoExterno: '.$codigoExterno, 500, "NO APLICA");
        }
    ...
    function notificar($idFactura, $estado, $idUsuario, $idEstacion, $idMotivoAnulacion, $observacion)
    {
        ...
        $urlServicio = $this->serviceUrl->retorna_rutaWS($this->idRestaurante, "TICTUK NOTIFICATIONS", "ANULACION");
        $this->request->url = $urlServicio["urlwebservice"];
