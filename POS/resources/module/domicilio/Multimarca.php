...
class Multimarca extends sql
{
    ...
    function solicitarToken()
    {
        ...
        $urlServicio = $this->serviceUrl->retorna_rutaWS($this->idRestaurante, "MULTIMARCA", "AUTHENTICATION");
        $this->request->url = $urlServicio["urlwebservice"];

        $client = $this->config->cargarConfiguracionPoliticasPorMedio($this->idRestaurante, "MULTIMARCA");
    ...
    function cambioEstado($codigoApp, $estado)
    {
        ...
        $urlServicio = $this->serviceUrl->retorna_rutaWS($this->idRestaurante, "MULTIMARCA", "CAMBIO ESTADO");
        $this->request->url = $urlServicio["urlwebservice"];
        ...
        if (isset($data) && isset($data->status) && $data->status === "success") {
            $this->insertaAuditoria($urlServicio["urlwebservice"], 'MULTIMARCA - Cambio a Estado: ' . $estado . ', CodigoApp: ' . $codigoApp, $this->response->httpStatus, json_encode($data));
        } else {
            $this->insertaAuditoria($urlServicio["urlwebservice"], 'MULTIMARCA - Cambio a Estado: ' . $estado . ', CodigoApp: ' . $codigoApp, 'ERROR', json_encode($data). ' - '. json_encode($this->response) );
        }
    ...
    function notificarTransferencia($codigo, $localOrigen, $localDestino, $usuario, $motivo, $direccion, $medio)
    {
        ...
        if(isset($validacionConsumoServicio) && $validacionConsumoServicio == 'APLICA'){
            ...
            // OBTENER RUTA
            $urlServicio = $this->serviceUrl->retorna_rutaWS($this->idRestaurante, "MULTIMARCA", "TRANSFERENCIA");
    ...
    function obtenerTokenPickupTrade($usuario, $idUsuario) {
        $urlServicio = $this->serviceUrl->retorna_rutaWS($this->idRestaurante, "PICKUP TRADE", "OBTENER TOKEN");
        try {
            $error = new \stdClass();
            if ( $urlServicio["estado"] == 1 ) {
                $client = $this->config->cargarConfiguracionPoliticasPickupAutenticacionTrade($this->idCadena);
                ...
                if (!isset($status) || ($status != 200 && $status != 201)) {
                    if($status == 404){
                        $error->mensaje = "Error Ruta de autenticacion trade " .   $url . " No valida ";
                        $error->codigo = 0;
                        $this->config->guardarAuditoria( $this->idCadena, $this->idRestaurante, $idUsuario, 0, 'OBTENER TOKEN TRADE', 'TRANSFERIR PEDIDO PICKUP', $request, json_encode($error) );
                        return json_encode($error);    
                    }
                    if (isset($response->message)) {
                        $error->mensaje = "Error: ". $status . " " . $response->message;
                    } else {
                ...
                $error->mensaje = "Servicio de obtener token trade no configurado.";
                $error->codigo = 0;
                $this->config->guardarAuditoria( $this->idCadena, $this->idRestaurante, $idUsuario, 0, 'OBTENER TOKEN TRADE', 'TRANSFERIR PEDIDO PICKUP', $request, json_encode($error) );
                return json_encode($error);
            }
    ...
    function notificarTransferenciaPickup($codigo, $localOrigen, $localDestino, $usuario, $motivo, $direccion, $idUsuario)
    {
        try {
            $error = new \stdClass();
            $request = json_encode(array(
            ...
            $data_token = $this->obtenerTokenPickupTrade($usuario, $idUsuario);
            if (isset($data_token->accessToken)) {
                $token = $data_token->accessToken;
                $urlServicio = $this->serviceUrl->retorna_rutaWS($this->idRestaurante, "PICKUP TRADE", "TRANSFERENCIA");
                if ( $urlServicio["estado"] == 1 ) {
                    $headers = array();
                    $headers[] = 'Content-Type: application/json';
                    ...
                    if (!isset($status) || ($status != 200 && $status != 201)) {
                        if($status == 404){
                            $error->mensaje = "Error ruta de transferencia trade " .   $url . " No valida ";
                            $error->codigo = 0;
                            $this->config->guardarAuditoria( $this->idCadena, $this->idRestaurante, $idUsuario, 0, 'TRANSFERENCIA TRADE', 'TRANSFERIR PEDIDO PICKUP', $request, json_encode($error) );
                            return json_encode($error);    
                        }
                        if (isset($response->message)) {
                            $error->mensaje = "Error: ". $status . " " . $response->message;
                    ...
                    if ( $status == 200 || $status == 201) {
                        $resp = array("Transaccion" => "OK", "codigo" => 200);
                    ...
                    $error->mensaje = "Servicio de transferencia pickup trade no configurado.";
                    $error->codigo = 0;
                    $this->config->guardarAuditoria( $this->idCadena, $this->idRestaurante, $idUsuario, 0, 'TRANSFERENCIA TRADE', 'TRANSFERIR PEDIDO PICKUP', $request, json_encode($error) );
                    return json_encode($error);
                }
            }else {
                $error->mensaje = "Token invalido";
                $error->codigo = 0;
                $this->config->guardarAuditoria( $this->idCadena, $this->idRestaurante, $idUsuario, 0, 'TRANSFERENCIA TRADE', 'TRANSFERIR PEDIDO PICKUP', $request, json_encode($error) );
                return json_encode($error);
            }
    ...
    function tracking($objetoTracking, $medio)
    {
        ...
        if(isset($validacionConsumoServicio) && $validacionConsumoServicio == 'APLICA'){
            ...
            if (isset($data) && isset($data->status) && $data->status === "success") {
                ...
                $this->insertaAuditoria($urlServicio["urlwebservice"], 'TRADE  - TRACKING - Cambio a Estado: Por Asignar' . ', ObjetoTracking: ' . json_encode($objetoTracking), 'ERROR', json_encode($data).''.json_encode($this->response) );
            }
    ...
    function cambioEstadoTradePorFactura($cdn_id, $cfac_id, $estado)
    {
        ...
        if ($this->fn_ejecutarquery($query)) {
            ...
            $this->insertaAuditoria($urlServicio["urlwebservice"], 'MULTIMARCA - Cambio a Estado: ' . $estado . ', CodigoApp: NO EXISTE', 'ERROR', 'No existe un codigo de App asociado a la factura ' . $cfac_id);
    ...
    function obtenerRutaArchivoToken()
    {
        ...
        $file = $ruta . 'multimarcaTokenStore.json';
        return $file;
    }
