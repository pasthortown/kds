...
class Callcenter extends sql
{
    ...
    function solicitarToken()
    {
        ...
        $client = $this->config->cargarConfiguracionPoliticasPorMedio($this->idRestaurante , 'CALLCENTER');
    ...
    function cambioEstado($codigoApp, $estado)
    {
        ...
        if (isset($data) && isset($data->status) && $data->status === "success") {
            $this->insertaAuditoria($urlServicio["urlwebservice"], 'CALLCENTER - Cambio a Estado: ' . $estado . ', CodigoApp: ' . $codigoApp, $this->response->httpStatus, json_encode($data));
        } else {
            $this->insertaAuditoria($urlServicio["urlwebservice"], 'CALLCENTER - Cambio a Estado: ' . $estado . ', CodigoApp: ' . $codigoApp, 'ERROR', json_encode($data). ' - '. json_encode($this->response) );
        }
    ...
    function cambioEstadoTradePorFactura($cdn_id, $cfac_id, $estado)
    {
        ...
        if ($this->fn_ejecutarquery($query)) {
            ...
            $this->insertaAuditoria($urlServicio["urlwebservice"], 'CALLCENTER - Cambio a Estado: ' . $estado . ', CodigoApp: NO EXISTE', 'ERROR', 'No existe un codigo de App asociado a la factura ' . $cfac_id);
    ...
    function obtenerRutaArchivoToken()
    {
        ...
        $file = $ruta . 'callcenterTokenStore.json';
        return $file;
    }
