...
/**
 * Created by PhpStorm.
 * Autor: fabricio.costa  XD
 * Fecha: 16/11/2017
 * 
 * Modificado: esteban.canelos
 * Fecha: 21/08/2018
 */
...
class Validacion {
    ...
    public function validarRuc($numero = ''){
        $numero = (string) $numero;
        $estado = false;
        $longitudValida = (strlen($numero) == 13) ? true : false;
        $estado = ($longitudValida && substr($numero, -3) == '001') ? true : false;
        return $estado;

        $this->setError('');
        $numVerficador = $numero[2];
        //echo gettype($numVerficador);
        /*switch ($numVerficador) {
            case ($numVerficador < 6):
     ...
     *
     * @param  string  $numero  Número de RUC sociedad pública
     *
     * @return Boolean
     */
     ...
     * @return boolean
     *
     * @throws exception Cuando el código de provincia no está entre 01 y 24, o es diferente de 30
     */
    protected function validarCodigoProvincia($numero) {
        if (!(($numero > 0 AND $numero <= 24) OR ($numero == 30))) {
            throw new Exception('Código de provincia (dos primeros dígitos) no debe ser mayor a 24 ni menor a 1, o diferente de 30');
        }
        return true;
    }
     ...
     * Los posibles valores del campo tipo son: cedula, ruc_natural, ruc_privada, ruc_publica
     *
     * Para cédulas no hay validación
     * 
     * Para RUC de personas naturales el tercer dígito debe
     * estar entre 0 y 5 (0,1,2,3,4,5)
     *
     * Para RUC de sociedades privadas el tercer dígito debe ser igual a 9.
     *
     * Para RUC de sociedades públicas el tercer dígito debe ser igual a 6.
     *
     ...
     *
     * Paso 2: Si alguno de los resultados de cada multiplicación es mayor a o igual a 10,
