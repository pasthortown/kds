////////////////////////////////////////////////////////////////////////////////////////////////////////
////////DESARROLLADO POR: CHRISTIAN PINTO///////////////////////////////////////////////////////////////
///////////DESCRIPCION: ADMINISTRACION DE MOTIVO ANULACION, LISTADO, AGREGAR Y MODIFICAR    ////////////
////////////////TABLAS: Motivo_Anulacion ////////////////////////////////////////////////////////////////
////////FECHA CREACION: 13/07/2015//////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////////

var pnt_Id = 0;
var accion = 0;
$(document).ready(function () {
    fn_btn('agregar', 1);
    //fn_cargarMotivoanulacion();
    fn_cargarMotivoanulacionInactivos(7);
});

function fn_OpcionSeleccionada(ls_opcion) {
    if (ls_opcion == 'Todos') {
        fn_cargarMotivoanulacion();
    } else if (ls_opcion == 'Activos') {
        accion = 7;
        fn_cargarMotivoanulacionInactivos(accion);
    } else if (ls_opcion == 'Inactivos') {
        accion = 8;
        fn_cargarMotivoanulacionInactivos(accion);
    }
}

function fn_OpcionSeleccionadaModInsert() {
    var ls_opcion = '';
    ls_opcion = $(":input[name=estados]:checked").val();

    if (ls_opcion == 'Todos') {
        fn_cargarMotivoanulacion();
    } else if (ls_opcion == 'Activos') {
        accion = 7;
        fn_cargarMotivoanulacionInactivos(accion);
    } else if (ls_opcion == 'Inactivos') {
        accion = 8;
        fn_cargarMotivoanulacionInactivos(accion);
    }
}

/*===================================================*/
/*FUNCION PARA TRAER LOS BOTONES DE ADMINISTRACION   */
/*===================================================*/
function fn_btn(boton, estado) {
    if (estado) {
        $("#btn_" + boton).css("background", " url('../../imagenes/admin_resources/" + boton + ".png') 14px 4px no-repeat");
        $("#btn_" + boton).removeAttr("disabled");
        $("#btn_" + boton).addClass("botonhabilitado");
        $("#btn_" + boton).removeClass("botonbloqueado");
    } else {
        $("#btn_" + boton).css("background", " url('../../imagenes/admin_resources/" + boton + "_bloqueado.png') 14px 4px no-repeat");
        $("#btn_" + boton).prop('disabled', true);
        $("#btn_" + boton).addClass("botonbloqueado");
        $("#btn_" + boton).removeClass("botonhabilitado");
    }
}

/*==========================================================*/
/*FUNCION PARA CARGAR AL INICIO TABLA DE MOTIVO ANULACION   */
/*==========================================================*/
function fn_cargarMotivoanulacion() {
    accion = 1;
    var html = "<thead><tr class='active'><th style='text-align:center; width:60%'>Descripci&oacute;n</th><th style='text-align:center'>Activo</th></thead>";
    send = {"administracionMotivoanulacion": 1};
    send.accion = accion;
    $.getJSON("../adminMotivoAnulacion/config_motivoanulacion.php", send, function (datos) {
        if (datos.str > 0) {
            //$('#tabla_motivoanulacion').empty();
            $('#motivoanulacion').show();
            for (i = 0; i < datos.str; i++) {
                html = html + "<tr id='" + i + "' onclick='fn_seleccion(" + i + ",\"" + datos[i]['mtv_id'] + "\");' ondblclick='fn_selecciondoble(" + i + ",\"" + datos[i]['mtv_id'] + "\")'><td style='text-align:center;'>" + datos[i]['mtv_descripcion'] + "</td>";
                if (datos[i]['std_id'] == 7) {
                    html += "<td style='text-align: center;'><input disabled='disabled' type='checkbox' checked='checked' id='optiondetalle'></td></tr>";
                } else if (datos[i]['std_id'] == 8) {
                    html += "<td style='text-align: center;'><input type='checkbox' disabled='disabled'id='optiondetalle'></td></tr>";
                }
            }
            $('#tabla_motivoanulacion').html(html);
            $('#tabla_motivoanulacion').dataTable(
                    {//PROPIEDADES DE LA DATABLE CAMBIAMOS EL IDIOMA A ESPA�OL
                        'destroy': true
                    }
            );
            $("#tabla_motivoanulacion_length").hide();
            $("#tabla_motivoanulacion_paginate").addClass('col-xs-10');
            $("#tabla_motivoanulacion_info").addClass('col-xs-12');
            $("#tabla_motivoanulacion_length").addClass('col-xs-6');
        } else {
            alertify.error("No existen datos.");
            //$("#botonesTodos").hide();
            //$('#impresora').hide();
            //html+="<tr>";
            //html+="<td colspan='6'>No existen datos para esta cadena.</td></tr>";
            $("#tabla_motivoanulacion").html(html);
        }
    });
}


/*==================================================================================================*/
/*FUNCION PARA CARGAR TABLA DE MOTIVO ANULACION ACTIVOS O INACTIVOS DE ACUERDO AL ESTADO 16 Y 17     */
/*==================================================================================================*/
function fn_cargarMotivoanulacionInactivos(accion) {
    var html = "<thead><tr class='active'><th style='text-align:center; width:60%'>Descripci&oacute;n</th><th style='text-align:center'>Activo</th></thead>";
    send = {"administracionMotivoanulacion": 1};
    send.accion = accion;
    $.getJSON("../adminMotivoAnulacion/config_motivoanulacion.php", send, function (datos) {
        if (datos.str > 0) {
            //$('#tabla_motivoanulacion').empty();
            $('#motivoanulacion').show();
            for (i = 0; i < datos.str; i++) {
                html = html + "<tr id='" + i + "' onclick='fn_seleccion(" + i + ",\"" + datos[i]['mtv_id'] + "\");' ondblclick='fn_selecciondoble(" + i + ",\"" + datos[i]['mtv_id'] + "\")'><td style='text-align:center;'>" + datos[i]['mtv_descripcion'] + "</td>";
                if (datos[i]['std_id'] == 7) {
                    html += "<td style='text-align: center;'><input disabled='disabled' type='checkbox' checked='checked' id='optiondetalle'></td></tr>";
                } else if (datos[i]['std_id'] == 8) {
                    html += "<td style='text-align: center;'><input type='checkbox' disabled='disabled'id='optiondetalle'></td></tr>";
                }
            }
            $('#tabla_motivoanulacion').html(html);
            $('#tabla_motivoanulacion').dataTable(
                    {//PROPIEDADES DE LA DATABLE CAMBIAMOS EL IDIOMA A ESPA�OL
                        'destroy': true
                    }
            );
            $("#tabla_motivoanulacion_length").hide();
            $("#tabla_motivoanulacion_paginate").addClass('col-xs-10');
            $("#tabla_motivoanulacion_info").addClass('col-xs-12');
            $("#tabla_motivoanulacion_length").addClass('col-xs-6');
        } else {
            alertify.error("No existen datos.");
            //$("#botonesTodos").hide();
            //$('#impresora').hide();
            //html+="<tr>";
            //html+="<td colspan='6'>No existen datos para esta cadena.</td></tr>";
            $("#tabla_motivoanulacion").html(html);
        }
    });
}

function fn_seleccion(fila, codigo) {
    Cod_Motivoanulacion = codigo;
    $("#tabla_motivoanulacion tr").removeClass("success");
    $("#" + fila + "").addClass("success");
    $("#txt_idmotivoanulacion").val(Cod_Motivoanulacion);
}

function fn_selecciondoble(fila, codigo) {
    lc_control = 3;
    Cod_Motivoanulacion = codigo;
    $("#txt_idmotivoanulacion").val(Cod_Motivoanulacion);
    $("#tabla_motivoanulacion tr").removeClass("success");
    $("#" + fila + "").addClass("success");
    fn_cargarMotivoanulacionModifica(Cod_Motivoanulacion);
    $('#ModalMod').modal('show');
}

/*====================================================================*/
/*FUNCION PARA ACCIONAR SI ES NUEVO O MODIFICAR Y LLAMAR A LA MODAL   */
/*====================================================================*/
function fn_accionar(accion) {
    if (accion == 'Nuevo') {
        $('#txt_descripcion').val('');
        lc_control = 2;
        $('#ModalNuevo').modal('show');
        ////////////Colocar foco en cualquier campo de la modal////////////////// CP
        $('#ModalNuevo').on('shown.bs.modal', function () {
            $("#txt_descripcion").focus();
        });
    } else if (accion == 'Grabar') {
        if (lc_control == 2) {
            if ($("#txt_descripcion").val() == '') {
                alertify.error("Ingrese Descripci&oacute;n.");
                return false;
            }
            fn_guardarMotivoanulacion();
            //fn_OpcionSeleccionadaModInsert();
        } else if (lc_control == 3) {
            if ($("#txt_descripcionMod").val() == '') {
                alertify.error("Ingrese Descripci&oacute;n.");
                return false;
            }
            fn_guardarMotivoanulacionModifica();
            //	fn_OpcionSeleccionadaModInsert();															
        }
    }
}

/*=====================================================================*/
/*FUNCION PARA GUARDAR UN NUEVO REGISTRO DE MOTIVO ANULACION           */
/*=====================================================================*/
function fn_guardarMotivoanulacion() {
    accion = 2;
    send = {"nuevoMotivoanulacion": 1};
    send.accion = accion;
    send.descripcion = $("#txt_descripcion").val();

    if ($("#option").is(':checked')) {
        send.estado = 7;
    } else {
        send.estado = 8;
    }
    $.getJSON("../adminMotivoAnulacion/config_motivoanulacion.php", send, function (datos) {
        alertify.success("Motivo de Anulaci&oacute;n agregado correctamente.");
        fn_OpcionSeleccionadaModInsert();
    });
    $('#ModalNuevo').modal('hide');
    $("#botonesTodos").show();
}

/*==================================================================*/
/*FUNCION PARA TRAER DATOS CUANDO MODIFICAMOS UN REGISTRO           */
/*==================================================================*/
function fn_cargarMotivoanulacionModifica(codigo) {
    //$('#titulomodalMod').html("Modificar Configuracion de Impresora: ");
    accion = 3;
    send = {"cargarMotivoanulacionMod": 1};
    send.mtv_id = codigo;
    send.accion = accion;
    $.getJSON("../adminMotivoAnulacion/config_motivoanulacion.php", send, function (datos) {
        if (datos.str > 0) {
            $('#titulomodalMod').html(" " + datos[0]['mtv_descripcion'] + " ");
            $('#txt_descripcionMod').val(datos[0]['mtv_descripcion']);
            $("#optionmod").empty();
            if (datos[0]['std_id'] == 7) {
                $("#optionmod").prop("checked", true);  // para poner la marca
            }
            if (datos[0]['std_id'] == 8) {
                $("#optionmod").prop("checked", false);
            }
        }
    });
}

/*=============================================================================================*/
/*FUNCION PARA GUARDAR DATOS CUANDO VAMOS A MODIFICAR UN REGISTRO                              */
/*=============================================================================================*/
function fn_guardarMotivoanulacionModifica() {
    accion = 4;
    //alert($("#txt_idcanalesimpresion").val());
    send = {"guardaMotivoanulacionMod": 1};
    send.accion = accion;
    send.descripcion = $("#txt_descripcionMod").val();
    send.mtv_id = $("#txt_idmotivoanulacion").val();

    if ($("#optionmod").is(':checked')) {
        send.estado = 7;
    } else {
        send.estado = 8;
    }
    $.getJSON("../adminMotivoAnulacion/config_motivoanulacion.php", send, function (datos) {
        alertify.success("Motivo de Anulaci&oacute;n actualizado correctamente.");

        fn_OpcionSeleccionadaModInsert();
    });
    $('#ModalMod').modal('hide');
}