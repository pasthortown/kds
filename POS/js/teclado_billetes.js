...
function Keypad() {
	...
	this.addKeyDef('ENTER', 'enter', function(inst) { plugin._selectValue(inst, '\x0D'); plugin._hidePlugin(); }, true);
	this.addKeyDef('ENTER2', 'enter2', function(inst) {
		plugin._curInst = (inst._inline ? inst : plugin._curInst);
		plugin._hidePlugin();
	});
	
	this.addKeyDef('TAB', 'tab', function(inst) { plugin._selectValue(inst, '\x09'); }, true);
	this.addKeyDef('X', 'vacio', function(inst) { plugin._vacioValue(inst); });
	...
	this.regional[''] = { // Default regional settings
		...
		clearText: '&larr;', // Display text for clear link
		clearStatus: 'Borra todos los caracteres', // Status text for clear link
		backText: '&larr;', // Display text for back link
		...
		enter2Text: 'OK', // Display text for carriage return
		enter2Status: 'Aceptar', // Status text for carriage return
		tabText: '→', // Display text for tab
		tabStatus: 'Horizontal tab', // Status text for tab
		tabText: '', // Display text for vacio
		tabStatus: '', // Status text for vacio
	...
	this._defaults = { // Global defaults for all the keypad instances
		...
		layout: ['789', '456', '123', '0' + this.ENTER2,this.CLEAR], // Layout of keys
