...
var seleccionarFilaTablaModificador = function (idModificador) {
    $("#tblDepartamentos tr").removeClass("success");
    $("#flModificador" + idModificador).addClass("success");
};

var cargarListaModificadores = function () {
    $('#mdl_rdn_pdd_crgnd').show();
    var html = "<thead><tr class='active'><th style='text-align:center'>Id Modificador</th><th style='text-align:center'>Modificador</th><th style='text-align:center' width='10%'>Estado</th></tr></thead>";
    send = {};
        ...
        success: function (datos) {
            if (datos.str > 0) {
                for (var i = 0; i < datos.str; i++) {
                    ...
                    html = html + "<tr id='flModificador" + datos[i]['idModificador'] + "' onclick=\"seleccionarFilaTablaModificador(\'" + datos[i]['idModificador'] + "\')\"><td class='text-center'>" + datos[i]['idModificador'] + "</td><td>" + datos[i]['Modificador'] + "</td><td style='text-align: center;'>" + estado + "</td></tr>";
                }
            }
...
var actualizarListaModificadores = function () {
    $('#mdl_rdn_pdd_crgnd').show();
    var html = "<thead><tr class='active'><th style='text-align:center'>Id Modificador</th><th style='text-align:center'>Modificador</th><th style='text-align:center' width='10%'>Estado</th></tr></thead>";
    send = {};
    send.metodo = "actualizarListaModificadores";
    $.ajax({async: false, type: "POST", dataType: "json", "Accept": "application/json, text/javascript2", contentType: "application/x-www-form-urlencoded; charset=utf-8; application/json", url: "../adminModificadores/config_modificadores.php", data: send,
        success: function (datos) {
            if (datos.str > 0) {
                for (var i = 0; i < datos.str; i++) {
        ...
        error: function (jqXHR, textStatus, errorThrown) {
            $('#mdl_rdn_pdd_crgnd').hide();
            alertify.error("Servicio no Disponible: " + errorThrown);
        }
    });
};
