...
function CalendarPicker(element, options) {
    // Core variables.
    this.date =  options.init || new Date();
    ...
    this.listOfAllDaysAsText = [...Array(7).keys()].map(this._toTranslatedWeekday(locale, options.showShortWeekdays)).map(
        (month) => {
            return month.charAt(0).toUpperCase() + month.slice(1);
        }
    )
    this.listOfAllMonthsAsText = [...Array(12).keys()].map(this._toTranslatedMonth(locale)).map(
        (month) => {
            return month.charAt(0).toUpperCase() + month.slice(1);
        }
    )
...
CalendarPicker.prototype._toTranslatedWeekday = function(locale, showShortWeekdays) {
    ...
    return function(dayOfWeekIndex) {
        return new Intl.DateTimeFormat(locale, { weekday: weekdayFormat })
            .format(new Date(2000, 1, dayOfWeekIndex))
    }
}

CalendarPicker.prototype._toTranslatedMonth = function(locale) {
    return function(monthIndex) {
        return new Intl.DateTimeFormat(locale, { month: 'long' })
            .format(new Date(2000, 0 + monthIndex, 1))
    }
}
...
CalendarPicker.prototype._insertDaysIntoGrid = function () {
    ...
    arrayOfDays.forEach(date => {
        ...
        var dateIsBetweenAllowedRange = (this.min || this.max) && date < this.min || date > this.max
