...
///////DESCRIPCION: Validación de parámetros y creación de nueva Promoción /////////////////
...
$(document).ready(function() {
    ...
    fn_cargarLocalesNoAsignadosDescuento(cdn_id);
        ...
        if (document.getElementById('anadirProductos').checked || document.getElementById('anadirDescuentos').checked) {
            ...
            if (document.getElementById('anadirProductos').checked) {
                ...
                var valoresNuevoElemento = {
                    ...
                    IDDescuento: $elementoPLU1.data("plu_id"),
                    cantidad_plu: 0,
                    concatenador_plu: Concatenador_beneficios,
                    Tipo_aplica: "FACTURA",
    ...
    $("#btnQuitarTodosRestaurantesCiudades").on("click", function() {});
    ...
    /*
        ...
        $("#horarioHasta").on("dp.change", function (e) {
            $('#horarioHasta').data("DateTimePicker").maxDate(document.getElementById("horarioDesde").value);
        });
    */
    ...
    $("#buscar_Ciudad").on('keyup', function(e) {
        filtrarCiudad();
    });

    $("#buscar_Region").on('keyup', function(e) {
        filtrarRegion();
    });
...
/**
 * Función verificarEdadHasta().
 * Verifica edades Desde Hasta
 * @return  {boolean}
 * @versión  3.0
 * @author Eduardo Valencia <educristo@gmail.com>
 * @copyright KFC
 **/
function verificarEdadHasta(a) {
    var edadHasta = parseInt(a.value);
    var edadDesde = parseInt(document.getElementById("edadDesde").value);

    if (edadDesde > edadHasta) {
        alertify.alert('Verifique el Rango de edades.');
        document.getElementById("edadHasta").value = parseInt(document.getElementById("edadDesde").value) + 1;
    }
 ...
 * Ejecuta el procedimiento almacenado [Descuentos].[Descuentos] para retornar los productos disponibles por cadena
...
function fn_cargarPlusDescuento(resultado, cdn_id, cla_id) {
...
/**
 * Función crearElementoListaPlusDescuento(elemento).
 * Completa el elemento <li> de los beneficios asignados a un CUPÓN/PROMOCIÓN
 * Verificar si el elemto tiene todos los campos de la entidad Beneficios
 * @elemento  {beneficios}
 * @return  {<li>}
 * @versión  3.0
 * @author Eduardo Valencia <educristo@gmail.com>
...
function crearElementoListaPlusDescuento(elemento) {
    ...
    var $elementoLista = $("<li id='Beneficio_" + elemento.plu_id + "' class='listado-plus-agregardos-descuento list-group-item'></li>");
    ...
    botonEliminarPlu.on("click", function() {
        ...
        beneficiosProductosCupon = beneficiosProductosCupon.replace(elemento.plu_id + ',' + elemento.cantidad_plu + ',' + elemento.Tipo_aplica + ',' + elemento.IDDescuento + ';', '');
...
function guardarPromo() {
    var totalCategorias = $('#totalCategorias').val();
    ...
    if (document.getElementById('Requiere_dias').checked) {
        ...
        var Requiere_dias = 1;
        var Dias_canjeable = lunes.concat(',', martes, ',', miercoles, ',', jueves, ',', viernes, ',', sabado, ',', domingo);
        Dias_canjeable = Dias_canjeable.replace(',,,,,,', ',');
        Dias_canjeable = Dias_canjeable.replace(',,,,,', ',');
        Dias_canjeable = Dias_canjeable.replace(',,,,', ',');
        Dias_canjeable = Dias_canjeable.replace(',,,', ',');
        Dias_canjeable = Dias_canjeable.replace(',,', ',');
        ...
        if (Dias_canjeable.charAt(0) == ',') {
            Dias_canjeable = Dias_canjeable.substring(1, Dias_canjeable.length);
        }
        if (Dias_canjeable.charAt(Dias_canjeable.length - 1) == ',') {
            Dias_canjeable = Dias_canjeable.slice(0, -1);
        }
    ...
    var categoriasSeleccionadasPromocion = categoriasPromocion.slice(0, -1);
 ...
 * @copyright KFC
 **/
function fn_cargarLocalesNoAsignadosDescuento(cdn_id) {
    var send;
    send = { "cargarRestaurantesTotal": 1 };
    send.cdn_id = cdn_id;
    $.getJSON("../adminDescuentos/config_descuentos.php", send, function(datos) {
        if (datos.str > 0) {
...
function crearItemListaRegiones(datosRestaurante) {
    ...
    var $botonAgregar = $("<button class='btn btn-xs btn-info' id='" + idB + "'> Agregar</button>");
    var $botonEliminar = $("<button class='btn btn-xs btn-danger' id='" + idB + "'> Eliminar</button>");
...
/**
 * Función btn_agregar_producto_requerido()
 * Añade un producto seleccionado a la lista de requerimientos para una promoción
 * Internamente verifica cantidad de producto añadido y que no existe duplicidad de productos
 * @return                {boolean}
 * @versión  3.0
 * @author Eduardo Valencia <educristo@gmail.com>
 * @copyright KFC
