 ...
 * HISTORIAL DE CAMBIOS:
    ...
    05/05/2025 George Vera
        Se incorpora el reporte de transferencia por factura
*/
/* global parseFloat */
...
function fn_muestraUsuariosEstado(fechaAperturaPeriodo, prd_id, banderaDesmontado) {
    ...
    let send = {"muestraUsuariosEstado": 1};
    send.accion = 1;
    ...
    send.prd_id = prd_id;
    $.ajax({async: false, type: "POST", dataType: "json", contentType: "application/x-www-form-urlencoded", url: "../adminDesmontarCajero/config_adminDesmontarCajero.php", data: send,
        success: function (datos) {
            if (datos.str > 0) {
                ...
                for (let i = 0; i < datos.str; i++) {
                    ...
                    if (i === 0) {
                        ...
                        html += "<div class=''></div><div class='row' style='width:220px'><button onclick='fn_generar_interface()' style='width:160px;' class='btn btn-success'><span class='glyphicon glyphicon-refresh' aria-hidden='true'></span>&nbsp;&nbsp;Interface SGWeb</button></div></td><td align='right' colspan=2><div class=''></div><div class='row' style='width:220px'><button id='' onclick='fn_reporteFinDeDia(\"" + prd_id + "\")' style='width:160px;' class='btn btn-success'><span class='glyphicon glyphicon-list' aria-hidden='true'></span>&nbsp;&nbsp;Reporte Fin de D&iacutea</button></div></td>";
                        html += "</tr><td>&nbsp;</td></tr>";
                    }
                    if (nColumnas < 9) {
                        html += "<td align='center'>";
                        html += "<div class=''><h6>" + datos[i]['usr_usuario'] + "</h6><img src='../../imagenes/admin_resources/icon-user_activo.png'/></div>";
						html += "<div class='btn-group'><div class='row' style='width:125px'>";
						if (datos[i]['esOperador']) {
							html += "<button style='width:90px;' class='btn btn-secondary btn-block' disabled>Operador</button>";
						} else {
							if (datos[i]['estado_usuario'] === 'Inactivo' && datos[i]['PorEnviar'] === 0) {
								html += "<button value= '" + datos[i]['estado_usuario'] + "' onclick='fn_formasPagoInactivo(\"" + datos[i]['usr_id'] + "\",\"" + datos[i]['ctrc_id'] + "\", \"" + datos[i]['usr_usuario'] + "\",\"" + prd_id + "\")' style='width:90px;' class='btn btn-default btn-block'>" + datos[i]['estado_usuario'] + "</button>";
							} else if (datos[i]['estado_usuario'] === 'Inactivo' && datos[i]['PorEnviar'] === 1) {
								html += "<button value= '" + datos[i]['estado_usuario'] + "' onclick='fn_formasPagoInactivo(\"" + datos[i]['usr_id'] + "\",\"" + datos[i]['ctrc_id'] + "\", \"" + datos[i]['usr_usuario'] + "\",\"" + prd_id + "\")' style='width:90px;' class='btn btn-default btn-warning'>" + datos[i]['estado_usuario'] + "</button>";
							} else if (datos[i]['estado_usuario'] === 'Activo') {
								html += "<button value= '" + datos[i]['estado_usuario'] + "' onclick='fn_formasPagoActivo(\"" + datos[i]['usr_id'] + "\", \"" + datos[i]['ctrc_id'] + "\", \"" + datos[i]['usr_usuario'] + "\",\"" + datos[i]['ctrc_usuario_desmontarcaja'] + "\",\"" + datos[i]['est_id'] + "\",\"" + prd_id + "\")' style='width:90px;' class='btn btn-success'>" + datos[i]['estado_usuario'] + "</button>";
							} else if (datos[i]['estado_usuario'] === 'Por Cerrar') {
								html += "<button value= '" + datos[i]['estado_usuario'] + "' onclick='fn_formasPagoPorCerrar(\"" + datos[i]['usr_id'] + "\",\"" + datos[i]['ctrc_usuario_desmontarcaja'] + "\", \"" + datos[i]['est_id'] + "\", \"" + datos[i]['ctrc_id'] + "\", \"" + datos[i]['usr_usuario'] + "\",\"" + prd_id + "\")' style='width:90px;' class='btn btn-info'>" + datos[i]['estado_usuario'] + "</button>";
							}
						}
						html += "</div></div></td>";
                    } else {
						...
						if (datos[i]['esOperador']) {
							...
							if (datos[i]['estado_usuario'] === 'Inactivo') {
								html += "<button value= '" + datos[i]['estado_usuario'] + "' onclick='fn_formasPagoInactivo(\"" + datos[i]['usr_id'] + "\",\"" + datos[i]['ctrc_id'] + "\", \"" + datos[i]['usr_usuario'] + "\",\"" + prd_id + "\")' style='width:90px;' class='btn btn-info'>" + datos[i]['estado_usuario'] + "</button>";
							} else if (datos[i]['estado_usuario'] === 'Activo') {
								html += "<button value= '" + datos[i]['estado_usuario'] + "' onclick='fn_formasPagoActivo(\"" + datos[i]['usr_id'] + "\", \"" + datos[i]['ctrc_id'] + "\", \"" + datos[i]['usr_usuario'] + "\",\"" + datos[i]['ctrc_usuario_desmontarcaja'] + "\",\"" + datos[i]['est_id'] + "\",\"" + prd_id + "\")' style='width:90px;' class='btn btn-success'>" + datos[i]['estado_usuario'] + "</button>";
							} else if (datos[i]['estado_usuario'] === 'Por Cerrar') {
								html += "<button value= '" + datos[i]['estado_usuario'] + "' onclick='fn_formasPagoPorCerrar(\"" + datos[i]['usr_id'] + "\",\"" + datos[i]['ctrc_usuario_desmontarcaja'] + "\", \"" + datos[i]['est_id'] + "\", \"" + datos[i]['ctrc_id'] + "\", \"" + datos[i]['usr_usuario'] + "\", \"" + prd_id + "\")' style='width:90px;' class='btn btn-danger'>" + datos[i]['estado_usuario'] + "</button>";
							}
							html += "</div></div></td>";
						}
                    }
    ...
    $.ajax({
        ...
        success: function (datos) {
            ...
            if (datos.str > 0) {
                ...
                htmlpie += "<div class='col-xs-2'><button type='button' id='eliminarCajasChicas' class='btn btn-warning' onclick=eliminarCajasChicasSIR('"+ usr_usuario +"') ><span class='glyphicon glyphicon-edit' aria-hidden='true'></span>&nbsp;&nbsp;Eliminar Cajas Chicas</button></div>";
...
function fn_formasPagoActivo(usr_id, ctrc_id, usr_usuario, usr_id_admin, est_id, prd_id) {
    ...
    html += "<div class='col-xs-2'><button type='button' id='eliminarCajasChicas' class='btn btn-warning' onclick=eliminarCajasChicasSIR('"+ usr_usuario +"') ><span class='glyphicon glyphicon-edit' aria-hidden='true'></span>&nbsp;&nbsp;Eliminar Cajas Chicas</button></div>";
        ...
        success: function (datos) {
            if (datos.str > 0) {
                for (j = 0; j < datos.str; j++) {
                    html = "<tr style='text-align:center; background-color:#00FF00'><td width='180' align='center' style='vertical-align:middle background-color:#00FF00'>" + datos[j]['fmp_descripcion'] + "</td>";
                    html += "<td style='text-align:center; background-color:#00FF00'>0.00</td>";
                    html += "<td style='text-align:center; background-color:#00FF00'>" + datos[j]['Transacciones'] + "</td>";
                    html += "<td style='text-align:center; background-color:#00FF00'>" + datos[j]['valor'] + "</td>";
                    html += "<td style='text-align:center; background-color:#00FF00'>0.00</td>";
                    html += "<td style='text-align:center; background-color:#00FF00'>0.00</td></tr>";
                ...
                if (banderaEstadoCajero === 'CajeroActivo') {
                    ...
                    $("#tabla_formaPagoPorCerrFar").append(html);
                } else if (banderaEstadoCajero === 'CajeroInactivo') {
                    $("#formaPago").append(html);
                }
            }
        },
        error: function (jqXHR, textFStatus, errorThrown) {
            alert(jqXHR);
            alert(textStatus);
            alert(errorThrown);
        }
...
function fn_generar_interface() {
    if (validar_restaurante(1) == 1) {
        if (servicio() == 1) {
                    ...
                    $.ajax({
                        ...
                        success: function (datos) {
                            ...
                            if (existe_diferencia == 'SI') {
                                ...
                                $("#alertify-ok").click(function() {
                                    //telegram(json, 1);
                                    enviarDatosInterface(IDPeriodo,interface_json,interface_eventos);
                                })           
                            } else {
                                enviarDatosInterface(IDPeriodo, interface_json, interface_eventos);  
                            }
            ...
            alertify.alert("El servicio para sincronizar venta por eventos no se encuentra activo.", function() {
                fn_cargando(0);
                alertify.message('OK');      
            });
        }
        ...
        success: function (datos) {
            if (datos.str > 0) {
                ...
                for (i = 0; i < datos.str; i++) {
					...
					$("#div_reporteFinDeDia").append(datos[i]['htmld']);
...
function enviarDatosInterface(IDPeriodo, interface_json, interface_eventos) {
    fn_cargando(1);
            ...
            $.ajax({
                ...
                //    const obj = JSON.parse(datos[0][0]);
                    ...
                    if (resp == 1 || resp == 0) {
                        ...
                        console.log(msj);
                        alertify.error("Existe problemas al realizar interface de venta, por favor comuniquese con soporte!");
                    }               
                },
                error: function (jqXHR, textStatus, errorThrown) {
                    fn_cargando(0);
...
function validar_restaurante(accion) {
    var send;    
    send = {"validar_eventos_restaurante": 1};//
    send.accion = accion; 
    var status=0;
    $.ajax({
        async: false,
...
function servicio() {
    ...
    $.ajax({
        ...
        dataType: "json",
        contentType: "application/x-www-form-urlencoded",
        url: "../../serviciosweb/interface/config_cliente_servicio_eventos.php",
        data: send,
        success: function (datos) {
            status=datos;
        }
...
function datos_conexion() {
    ...
    $.ajax({
        ...
        data: send,
        success: function (datos) {
           status["hostLocal"]=host_local(); 
           status["host"]=datos[0]["host"]; 
           status["database"]=datos[0]["database"];  
           status["username"]=datos[0]["username"];
           status["password"]=datos[0]["password"];   
        }
    });   
    return status;
}
function conexion(id) {
    ...
    send = {};//
    send.metodo='conexion';
    send.hostLocal=datos["hostLocal"];
    send.host=datos["host"];
    send.database=datos["database"];
    send.username=datos["username"];
    send.password=datos["password"];
    send.IDPeriodo=id;
    $.ajax({
        async: false,
        type: "POST",
        ...
        success: function (datos) {
            status=datos;
            alertify.alert("Información venta de eventos sincronizada a local correctamente.");  
        }
    });
    fn_cargando(0);
    return status;
...
function cargar_URL(tipoTransaccion) {
    ...
    $.ajax({
        ...
        error: function (jqXHR, textStatus, errorThrown) {
            ...
            $( "#lista_restaurantes" ).html(html_tabla);
        }
    });        
    return urlToReturn;
}
...
function eliminarCajasChicasSIR( nom_usr ){
    let fecha_periodo = $("#hide_fecha").val();
    periodo = $("#hide_periodo").val();
    let send = {"muestraUsuariosEstado": 1};
    send.accion = 1;
    send.fecha = cambiarFormatoFecha(fecha_periodo);
    send.prd_id = periodo;
    $.ajax({
        async: false, 
        type: "POST", 
        ...
        success: function (datos) {
            if (datos.str > 0) {
                let validadoInactivo = false
                for (let i = 0; i < datos.str; i++) {
                    let estadoUserPos = datos[i]['estado_usuario']
                    if( estadoUserPos === 'Inactivo' && !validadoInactivo ){                      
                        let url = cargar_URL('ELIMINAR');
                        let send = {
                            cajasChicasSIR: 1,
                            url: url,
                            accion: 1,
                            cod_cajero: nom_usr,
                            fecha: cambiarFormatoFecha(fecha_periodo),
                            localizacion : consultar_localizacion()
                        }
                        $.ajax({
                            async: false,
                            type: "GET",
                            ...
                            success: function( datos ) {
                               if( datos.cajas.length > 0 ){
                                if( datos.cajas[0]['estado'] == 1 ){
                                    alertify.error(datos.cajas[0]['mensaje']);
                                }else if(  datos.cajas[0]['estadoMix'] == 3 ){
                                   alertify.error(datos.cajas[0]['mensaje']);
                                }else if( datos.cajas[0]['estadoSINOExiste'] == 6 ){
                                    alertify.error( datos.cajas[0]['mensaje'] )
                                }else if( datos.cajas[0]['estadoImportado'] == 1  ){
                                    alertify.error( datos.cajas[0]['mensaje'] )
                                }else if( datos.cajas[0]['estadofinal'] == 5  ){
                                    alertify.success( datos.cajas[0]['mensaje'] ) 
                                    let send = {
                                        accion: 1,
                                        cod_cajero: nom_usr,
                                        fecha: cambiarFormatoFecha(fecha_periodo),
                                        idControlEstacion: $("#hid_controlEstacion").val(),
                                        eliminarRegistroCajaChica: 1
                                    }
                                    $.ajax({
                                        async: false,
                                        type: "POST",
                                        ...
                                        data: send,
                                        success: function( datos ) {
                                            fn_generar_interface(); 
                                        },
                                        error: function () {
                                            alert('Error al consumir endpoint...');
                                        }
                                    });                                    
                                }
                               }else{
                                   alertify.error( 'No existen datos de la caja chica para eliminar' )
                               }
                            },
                            error: function () {
                                alert('Error al consumir endpoint...');
                        ...
                        validadoInactivo = true
                    } 
                }
                if( !validadoInactivo ){
                    alertify.error('NO SE PUDO ELIMINAR LA CAJA CHICA PORQUE TODOS LOS USUARIOS ESTAN ACTIVOS')
                }
            } else {
                alertify.error("No existen usuarios logueados para este periodo.");
            }
        ...
        error: function () {
            alert('Error al consumir el endpoint...');
        }
    });
}
