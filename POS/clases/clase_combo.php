  ...
  $lc_query="SELECT pis_id, pis_numero FROM Pisos WHERE rst_id = $lc_datos[0] ORDER BY pis_numero"; 
   ...
   $lc_query="SELECT m.mesa_id, m.mesa_descripcion, m.mesa_coordenadax, m.mesa_coordenaday,e.std_descripcion
						FROM Restaurante r inner join Pisos p on r.rst_id=p.rst_id
							  ...
							  WHERE r.rst_id=".$lc_datos[0]." and p.pis_id=".$lc_datos[1]." and ap.arp_id=".$lc_datos[2]." AND m.mesa_coordenadax is not NULL"; 
   if($this->fn_ejecutarquery($lc_query))
   {
     while($row = $this->fn_leerarreglo())
     {
...
/////////////////////////////////////////////////////////////////////////
   case 'CargarCliente':
   $lc_query="SELECT cli_id, cli_nombres, cli_apellidos, cli_documento FROM Cliente WHERE cli_apellidos LIKE '%$lc_datos%' OR cli_nombres LIKE '%$lc_datos%' ORDER BY cli_apellidos"; 
	  
   if($this->fn_ejecutarquery($lc_query))
   {
     while($row = $this->fn_leerarreglo())
     {
      	$cliente= $row['cli_apellidos'].' ' .$row['cli_nombres'];
     	$this->lc_regs[] = array("cli_id"=>$row['cli_id'],"cli_nombre"=>$cliente,"value"=>$cliente,"cli_documento"=>$row['cli_documento']);        
      }
